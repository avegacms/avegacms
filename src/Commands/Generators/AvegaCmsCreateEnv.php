...
class AvegaCmsCreateEnv extends BaseCommand
{
    ...
    private function createNewEnvironmentFile(): bool
    {
        ...
        $env['environment'] = self::$envTypes[CLI::promptByKey(
            'Set environment:',
            self::$envTypes,
        ...
        if (empty($env['db']['hostname'] = CLI::prompt(
            'Set your database.hostname (by default localhost)',
            null,
            ['permit_empty']
        ))) {
        ...
        if (empty($env['db']['dbDriver'] = CLI::prompt(
            'Set your database.DBDriver (by default MySQLi)',
            null,
            ['permit_empty']
        ))) {
        ...
        if (empty($env['db']['dbPrefix'] = CLI::prompt(
            'Set your database.DBPrefix (by default empty)',
            null,
            ['permit_empty']
        ))) {
            $env['db']['dbPrefix'] = '';
        ...
        if (empty($env['db']['port'] = CLI::prompt(
            'Set your database.port (by default 3306)',
            null,
